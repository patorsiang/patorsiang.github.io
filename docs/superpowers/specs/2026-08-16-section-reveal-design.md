...
| Reduced motion           | No bespoke handling                                              | Same reliance pattern as the hero mark and the theme cross-fade: the sitewide `@media (prefers-reduced-motion: reduce)` block in `globals.css` already forces every transition to near-zero duration.                                                                                                                                                                                                                |
...
On mount, a `useEffect` creates an `IntersectionObserver` watching the wrapper div, with `threshold: 0` (any pixel visible counts) and no `rootMargin` adjustment. Its callback fires once, synchronously, with the element's current intersection state:
...
- **Not intersecting:** apply the hidden state (`opacity-0 translate-y-2`, no transition yet — this happens in the same tick as mount, before the user has had a chance to see the un-hidden version). Keep observing. The first time the observer reports the element has become intersecting, apply the visible state (`opacity-100 translate-y-0`) with a `transition` class, and disconnect the observer — one-shot, never re-hides on scroll back up.
...
3. Reduced-motion: covered by the existing sitewide sweep (`e2e/reduced-motion.e2e.ts`) once `RevealOnView` exists on the homepage, same reliance pattern already proven for the hero mark and theme cross-fade — verified by running that suite, not by a bespoke assertion.
