...
- Wire the existing tests into CI — `packages/cv-engine` has 58 passing tests across 7 files (`bun run --cwd packages/cv-engine test`), but the root `package.json` has no `test` script and none of the three workflows run them, so they have never guarded a commit. `nextjs.yml` runs only `bun run build`. Found 2026-08-03.
